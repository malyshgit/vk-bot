...
public class Main {
    ...
    private static class MainHandler implements HttpHandler {
        ...
        static{
            try {
                var is = Main.class.getResourceAsStream("/www/auth.html");
        ...
        public void handle(HttpExchange exchange) {
            try {
                ...
                if (Main.class.getResource(www + path) == null) {
                    bytes.write("404".getBytes());
                }else{
                    if(path.equals("auth.html")) {
                        bytes.write(indexHtmlString.getBytes(StandardCharsets.UTF_8));
                    }else bytes.write(IOUtils.toByteArray(Main.class.getResourceAsStream(www + path)));
                }
